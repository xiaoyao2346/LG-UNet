...
from lib.LGUNet_LU_02 import Network
        ...
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} Loss2: {:0.4f}'.
                      format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss.data, loss.data))
                logging.info(
                    '[Train Info]:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} '
                    'Loss2: {:0.4f}'.
                    format(epoch, opt.epoch, i, total_step, loss.data, loss.data, loss.data))
                # TensorboardX-Loss
                writer.add_scalars('Loss_Statistics',
                                   {'Loss_init': loss.data, 'Loss_final': loss.data,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/root/autodl-tmp/Dataset/PolypDataset/TrainDataset/',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='/root/autodl-tmp/Dataset/PolypDataset/TestDataset/CVC-ColonDB/',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str,
                        default='./snapshot/Polyp2/',
    ...
    # build the model
    model = Network(channel=64,imagenet_pretrained=False).to(device)
    # 如果有多个 GPU，使用 DataParallel 来并行训练
    if torch.cuda.device_count() > 1:
        print(f"Using {torch.cuda.device_count()} GPUs!")
        model = nn.DataParallel(model)  # 在多个 GPU 上并行训练
