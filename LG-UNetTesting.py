...
import cv2
from lib.LG-UNet import Network
...
parser.add_argument('--pth_path', type=str, default='./snapshot/full_LGUNet_newest_02/Net_epoch_best.pth')
opt = parser.parse_args()

for _data_name in ['CHAMELEON', 'CAMO', 'COD10K',  'NC4K']:
    data_path = '/root/autodl-tmp/Dataset/TestDataset/{}/'.format(_data_name)
    ...
    if torch.cuda.device_count() > 1:
        ...
        model = nn.DataParallel(model)  # 在多个 GPU 上并行推理
