...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        self.rfb5 = RFB_modified(channel, channel)
        self.rfb6 = RFB_modified(channel, channel)
        self.rfb7 = RFB_modified(channel, channel)
        self.rfb8 = RFB_modified(channel, channel)

        self.NCD2 = NeighborConnectionDecoder(channel)

        self.conv_concat2 = BasicConv2d(2*channel, channel, 1)
        self.conv_concat3 = BasicConv2d(3*channel, channel, 1)
        self.conv_concat4 = BasicConv2d(4*channel, channel, 1)
    ...
    def forward(self, x):
        ...
        x1 = self.rfb5(x1) + x1
        x1_d = F.interpolate(x1, size=x2.size()[2:], mode='bilinear')
        x2 = self.rfb6(x2 + x1_d) + x2
        x2_d = F.interpolate(x2, size=x3.size()[2:], mode='bilinear')
        x3 = self.rfb7(x3 + x2_d) + x3
        x3_d = F.interpolate(x3, size=x4.size()[2:],mode='bilinear')
        x4 = self.rfb8(x4 + x3_d) + x4

        # Neighbourhood Connected Decoder
        x4, x3, x2, x1, S_g = self.NCD1(x4, x3, x2, x1)
        x3 = self.conv_concat2(x3)
        x2 = self.conv_concat3(x2)
        x1 = self.conv_concat4(x1)

        x4, x3, x2, x1, S_g = self.NCD2(x4, x3, x2, x1)
