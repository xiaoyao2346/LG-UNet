...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.project_out(torch.cat((x_3,x_5,),1))
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        #out_3 = self.project_out(torch.cat((out_3, x1), 1))
        ...
        out = self.project_out(torch.cat((out_3,out_5),1))
        return out
...
class LocalUnet(nn.Module):
    def __init__(self, channel):
        ...
        self.mtb1 = MSA_head(dim=channel)
        self.mtb2 = MSA_head(dim=channel)
        self.mtb3 = MSA_head(dim=channel)
        # self.project_out = nn.Conv2d(dim*2, dim, kernel_size=1, bias=bias)
        self.conv_cat = nn.Conv2d(channel*2, channel, 1)

    def forward(self, x1, x2):
        x2_d = F.interpolate(x2, size=x1.size()[2:], mode='bilinear')
        x1 = self.mtb1(x1)
        x2_d = self.mtb2(x2_d)
        x = self.conv_cat(torch.cat((x1, x2_d), 1))
        x = self.mtb3(x)
        return x, F.interpolate(x, size=x2.size()[2:], mode='bilinear')
